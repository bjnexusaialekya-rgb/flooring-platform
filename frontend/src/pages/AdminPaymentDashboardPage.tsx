...
export function AdminPaymentDashboardPage() {
  const [payments, setPayments] = useState<PaymentStatusRow[] | null>(null);
  ...
  const metrics = useMemo(() => {
    const rows = payments ?? [];
    const totalAmount = rows.reduce((sum, p) => sum + (p.amount ? Number(p.amount) : 0), 0);
    const succeeded = rows.filter((p) => p.payment_status === 'succeeded');
    const succeededAmount = succeeded.reduce((sum, p) => sum + (p.amount ? Number(p.amount) : 0), 0);
    const outstandingAmount = totalAmount - succeededAmount;
    return {
      totalAmount,
      succeededAmount,
      outstandingAmount,
      ...
      <div className="bg-[var(--color-panel)] rounded-xl border surface-card border-[var(--color-concrete-light)] p-6">
        ...
        {payments !== null && payments.length === 0 && (
          <EmptyState
            ...
            description="Consolidated billing batches will appear here once work orders are completed and grouped for invoicing."
          />
        )}
